...
const email = ref('')//ref é pra variavel reativa 
const password = ref('')
...
//chama quando clica pra enviar
const handleSubmit = async () => {
  errorMessage.value = ''
  const showPassword = ref(false)
  const data = new URLSearchParams() //objeto criado que será enviado para o corpo da requisição 
  ...
  try {
    const response = await fetch('http://127.0.0.1:8000/o/token/', { //url pra requisição
      method: 'POST',
      body: data
    })
    ...
    if (!response.ok) {
      ...
      throw new Error(err.error_description || 'Login falhou')
    }

    const tokenData = await response.json()//converte a reposta em json
    console.log('Tokens recebidos:', tokenData)

    // salvar tokens
    localStorage.setItem('access_token', tokenData.access_token)//salva dados no navegador
